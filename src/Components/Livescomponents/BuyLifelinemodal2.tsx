  ...
  const getData = async () => {
    try {
      ...
      const result = await fetch(
        `${apiBaseUrl}/buy_lives_plan/${props.id}`,
        {
          method: 'POST',
          headers: {
      ...
      if (json.status === 'success') {
        ...
        setMg(json.message);
        dispatch(usertotalLive(json?.lives ? json?.lives : '0'));
        navigation.navigate('Webmodallive', {
          uri: json?.order?.noon_p_checkout_post_url,
        });
        await AsyncStorage.setItem(
          'Buylife_noon_orderid1',
...
export default BuyLifeLineModal2;
