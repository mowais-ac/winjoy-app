...
const AddConnectionModal = (props) => {
  ...
  const SearchData = async () => {
    ...
    if (text && text !== "" && !ButtonRef.current.GetActivity() && !AddBtn) {
      ...
      var Result;
      await fetch(
        `${apiBaseUrl}/connections/search?user_name=${text}`,
        requestOptions
      )
      ...
      if (Result === "error") setSearched("error");
        ...
        await fetch(
          `${apiBaseUrl}/connected/with/${Result.id}`,
          requestOptions
        )
          .then(async (response) => response.json())
          .then((res) => {
        ...
        if (Result.added === 0) {
          await fetch(
            `${apiBaseUrl}/connection-request/sent/to/${Result.id}`,
            requestOptions
          )
            .then(async (response) => response.json())
            .then((res) => {
        ...
        if (Result.added === 0) {
          await fetch(
            `${apiBaseUrl}/connection-request/from/${Result.id}`,
            requestOptions
          )
            .then(async (response) => response.json())
            .then((res) => {
  ...
  const SendRequest = async () => {
    ...
    await fetch(
      `${apiBaseUrl}/send/connection-request/${Searched.id}`,
      requestOptions
    )
      .then(async (response) => response.json())
      .then((res) => {
