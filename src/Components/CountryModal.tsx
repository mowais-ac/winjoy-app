...
const CountryModal = (props: any) => {
  ...
  useEffect(() => {
   ...
   // if (props.CountryRef) props.CountryRef.current = setIsVisible;
    // if (!!countrydata?.data) {
    //   setData(countrydata?.data?.countries?.sort((a, b) => (a?.name > b?.name ? 1 : -1)));
    // }
    // const check = async () => {
    //   try {
    //     if (Data === null) {
  ...
  const HandleTextChange = t => {
    if (!t || t === '') return setFiltered(null);
      ...
      setFiltered(
        countrydata?.data?.countries?.filter(x => x.name.toLowerCase().indexOf(t.toLowerCase()) !== -1),
      );
  };
