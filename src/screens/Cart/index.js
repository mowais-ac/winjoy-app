...
const index = ({ navigation }) => {
  ...
  const GetData = async () => {
    ...
    let dat2 = await AsyncStorage.getItem('ids');
   ...
   if(expData!==null){
    ...
    JSON.parse(expData).map(element => {
      ids =ids+element.product_id+ ",";
    });
    console.log("idss",ids);
   }
    if (dat2 !== null) {
      
      JSON.parse(dat2).map(element => {
        id = id + element + ",";
      });
    }
    let isActive = true;
    ...
    const check = async () => {
      ...
      if (Data?.length <= 0) {
        ...
        await fetch(`${Config.API_URL}/addtocart/products/collection?id=${ids + id}`, requestOptions)
  ...
  const RemoveItem = async (id) => {
    ...
    let dat2 = await AsyncStorage.getItem('expData');
    //  let data=[1,2,3,4,5]
    ...
    let array2 = JSON.parse(dat2);
    const index2 = array2.indexOf(id);
    if (index2 > -1) {
      array2?.splice(index2, 1);
    }

    AsyncStorage.setItem('expData', JSON.stringify(array))
    AsyncStorage.setItem('ids', JSON.stringify(array2))
