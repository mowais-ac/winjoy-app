...
import { homeData } from '../../reduxToolkit/actions/home.actions';
import { walletData } from '../../reduxToolkit/actions/wallet.actions';
import { winnersData } from '../../reduxToolkit/actions/winners.actions';
import { livedrawData } from '../../reduxToolkit/actions';
const LiveProducts = ({route, navigation}:any) => {
  const Isfocused = useIsFocused();
  const LandingData = useSelector(state => state?.home?.homeData);
  const livedrawdata = useSelector(state => state.livedraw?.livedrawData);
  const userData = useSelector(state => state.user.userData);
  const Winnerdata = useSelector(state => state.winners.winnersData);
  ...
  useEffect(() => {
    dispatch1(homeData());
    Liveluckydraw(livedrawData());
    dispatch3(winnersData(livedrawdata.ldata?.ive_luckydraw?.id));
    dispatch4(walletData());
  ...
  const GetData = async () => {
    ...
    try {
      ...
      console.log(Token);
      const data = {live_luckydraw_id: livedrawdata?.data?.live_luckydraw?.id};
  ...
  const SendOrderId = async () => {
    try {
      ...
      const result = await fetch(`${apiBaseUrl}/order/pay_now_buy_live`, {
        method: 'POST',
        headers: {
  ...
  const OrderDetail = async () => {
    ...
    const Token = await EncryptedStorage.getItem('Token');
    const body = JSONtoForm({
      product_id: livedrawdata?.data?.live_luckydraw?.product?.product_id,
    ...
    await fetch(`${apiBaseUrl}/fanjoy/order/create`, requestOptions)
  ...
  const JoinUsers = async () => {
    ...
    const Token = await EncryptedStorage.getItem('Token');
    const body = JSONtoForm({
      live_luckydraw_id: livedrawdata?.data?.live_luckydraw?.id,
    });
    const requestOptions = {
      method: 'POST',
      headers: {
    ...
    await fetch(
      `${apiBaseUrl}/web/liveluckydraw/participants`,
      requestOptions,
    )
      .then(async response => response.json())
      .then(res => {
  ...
  const Payment_noon = async () => {
    ...
    await fetch(`${apiBaseUrl}/order/create`, requestOptions)
      .then(async response => response.json())
      .then(async res => {
        try {
  ...
  // console.log('livedrawdata',data?. Winnerdata?.data?.data?.winner);
  return (
    <View style={styles.gradientView}>
      ...
      {checkLive && (
        <LinearGradient
          ...
          {Itemshow2 && (
            <Chat
              data={livedrawdata?.data?.live_luckydraw?.product}
              userInfo={userData}
              loading={Loader}
              onPress={() => {
      ...
      />
      <WinnerModal
        Winner={Winnerdata?.data?.data?.winner}
