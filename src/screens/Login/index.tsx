...
import {setUserData, usertotalLive} from '../../reduxToolkit/slices/userSlice';
...
const index = ({navigation}: any) => {
  ...
  const ModalStateLanguage = useRef<any>();
  ...
  const LanguagePost = async () => {
    ...
    await fetch(`${apiBaseUrl}/user/update_language`, requestOptions)
      .then(async response => response.json())
      .then(async res => {
        console.log('resLang', res);
        if (res.message === 'Language has been updated') {
          dispatch(setUserData(res?.data?.user));
          dispatch2(usertotalLive(res?.data?.user?.lives_count));
  ...
  const HandleLogin = async () => {
    if (
      ...
      await fetch(
        `https://apiwinjoy.incubytes.com/api/auth/login`,
        requestOptions,
      )
        .then(async response => response.json())
        .then(async res => {
