...
const index = ({route, navigation}) => {
  ...
  const HandlePersonal = async () => {
    ...
    if (pictureref.current !== null) {
      ...
      await fetch(`${apiBaseUrl}/update/user/profile-image`, requestOptions)
  ...
  const HandleSettings = async () => {
    if (
      ...
      await fetch(`${apiBaseUrl}/user/password-change`, requestOptions)
        .then(async response => response.json())
        .then(async res => {
          if (res.status && res.status.toLowerCase() === 'success') {
    ...
    return (
      <>
        ...
        <View style={styles.InputView}>
          <Label notAlign darkmuted text="Country" style={{marginVertical:width*0.025}}  />
          <GetCountrySection />
        </View>
        <GetField
          name="City"
      ...
      <SafeArea>
        <LinearGradient
          ...
          <View style={styles.MainTop}>
            {userData !== null && (
              <UserInfo style={styles.header} OwnUser popup status={userData?.status} />
            )}
          </View>
          {OldUser && <GetData />}
