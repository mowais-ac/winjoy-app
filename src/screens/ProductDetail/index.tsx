...
import { CART_COUNTER } from '../../reduxToolkit/slices/cartSlice';
const MYServer = 'https://node-winjoyserver-deploy.herokuapp.com/';
const ProductDetail = ({props, navigation, route}: any) => {
  ...
  const _Api = async productId => {
    ...
    await fetch(`${apiBaseUrl}/product/show/${productId}`, requestOptions)
  ...
  const SaveIdInfo = async () => {
    if (!pd?.product?.luckydraw.enable_buy) {
      ...
      await fetch(`${apiBaseUrl}/add_to_cart`, requestOptions)
        .then(async response => response.json())
        .then(async res => {
          if (res.status === 'success') {
            ...
            dispatch(CART_COUNTER(counterMain + count));
          } else {
            setActivity(false);
