...
import {experiencecelebData} from '../../reduxToolkit/actions/expericnecelebSlug.actions';
...
const index = ({route, navigation}: any) => {
  ...
  const creatorExpList = useSelector(
    state => state.experienceceleb.creatorExpList,
  );
  useEffect(() => {
    dispatch(experiencecelebData(experience_id));
  }, []);
    ...
    <SafeAreaView style={styles.safeStyle}>
      <ScrollView>
        <LinearGradient
          ...
          style={{height: height}}>
          <ImageBackground
            source={{uri: creatorExpList?.data?.experience?.cover_photo}}
            ...
            <View style={{marginTop: height * 0.069}}>
              <Text
                ...
                {creatorExpList?.data?.experience?.title}
              </Text>
              <Text
                style={{
                  color: '#ffffff',
                ...
                {creatorExpList?.data?.experience?.short_desc}
              </Text>
            </View>
          </ImageBackground>
          ...
          <View
            ...
            <FlatList
              data={creatorExpList?.data?.experience_celebrities}
              numColumns={2}
              //style={{paddingLeft: 8}}
              renderItem={({item}) => (
                <CreatorExperienceCard
