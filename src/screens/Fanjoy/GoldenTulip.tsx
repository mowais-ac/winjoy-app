...
const GoldenTulip = ({navigation, slug, route, props}:any) => {
  const SlugDetails = useSelector(state => state.causesSlug.causesSlugData);
  const totalLives = useSelector(state => state.user.totalLives);
  const Homedetails = useSelector(state => state.livedraw.livedrawData);
  ...
  const OrderDetail = async () => {
    ...
    const body = JSONtoForm({
      product_id: SlugDetails?.data?.data?.product?.product_id,
  ...
  const JoinUsers = async () => {
    ...
    const Token = await EncryptedStorage.getItem('Token');
    const body = JSONtoForm({
      live_luckydraw_id: Homedetails?.data?.live_luckydraw?.id,
    });
    const requestOptions = {
      method: 'POST',
      headers: {
  ...
  //console.log('s', SlugDetails?.data?.data);
  ...
  return (
    <ScrollView style={styles.srollbody}>
      <View style={styles.mainbody}>
        <ImageBackground
          resizeMode="cover"
          source={{uri: SlugDetails?.data?.data?.product?.image}}
          style={styles.bgimg}>
          <Header style={styles.header} />
          <View style={styles.topbody}>
            <View style={styles.topinnerbody}>
              <Text style={styles.toptext1}>
                {SlugDetails?.data?.data?.product?.title}
              </Text>
              <Text style={styles.toptext3}>
                {SlugDetails?.data?.data?.product?.description}
              </Text>
            </View>
            <LinearGradient
              start={{x: 0, y: 0}}
