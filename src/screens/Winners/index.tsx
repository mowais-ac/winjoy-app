...
import { gameshowwinnersData, livePlanData } from '../../reduxToolkit/actions';
import { luckydrawwinnersData } from '../../reduxToolkit/actions/luckydrawwinners.action';
...
const index = ({route, navigation}:any) => {
  ...
  const gameShowWinners = useSelector(state => state.gameshowwinners.gameshowwinnersData);
  const luckyDrawWinners = useSelector(state => state.luckydrawwinners.luckydrawwinnersData);
 // const loading = useSelector(state => state.event.loading);
  ...
  const gamewinnersdispatch = useDispatch();
  const luckydrawwinnerdispatch2 = useDispatch();
  const routeSelected = route?.params?.selected;
  useEffect(() => {
    //dispatch(livePlanData());
    gamewinnersdispatch(gameshowwinnersData())
    luckydrawwinnerdispatch2(luckydrawwinnersData())
      ...
      <LinearGradient
        ...
        <ScrollView
          ...
          <View style={{width: '100%', alignItems: 'center'}}>
            {gameShowWinners?.loading ? (
              <ActivityIndicator size="large" color="#000000" />
            ) : (
              <>
                {selected === 0 ? (
                  <GameShow
                    lastWinners={gameShowWinners?.data?.winners}
                    pastWinners={gameShowWinners?.data?.pastWinners}
                    navigation={navigation}
                  />
                ) : (
                  <LuckyDraw
                    winnersLastGame={luckyDrawWinners?.data?.winners}
                    navigation={navigation}
                  />
                )}
              </>
