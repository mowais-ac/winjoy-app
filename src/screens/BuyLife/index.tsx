...
import { livePlanData } from '../../reduxToolkit/actions/livePlans.action';
import { usertotalLive } from '../../reduxToolkit/slices/userSlice';
  ...
  const getData = async () => {
    try {
      ...
      const result = await fetch(
        `${apiBaseUrl}/buy_lives_plan/${idVideoAdd}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'multipart/form-data',
      ...
      if (json.status === 'success') {
        ...
        rewardAd.show();
        setInterval(() => {
          dispatch2(usertotalLive(json?.lives));
        }, 35 * 1000);
        liveplansDispatch(livePlanData())
      }
    } catch (error) {
    ...
    <SafeAreaView style={styles.safeStyle}>
      <ScrollView
        ...
        <LinearGradient
          ...
          style={{paddingBottom: 15, minHeight:900}}>
          <LinearGradient
            start={{x: 0, y: 0}}
            end={{x: 1, y: 0}}
