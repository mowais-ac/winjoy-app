...
const Landing = (props: any) => {
  ...
  //  const LandingData?.data = useSelector(state => state.app.LandingData);
  ...
  const celebsListDispatch = useDispatch();
  const fanjoyDispatch = useDispatch();
  const leaderboardDispatch = useDispatch();
  const liveplansDispatch = useDispatch();
  const walletDispatch = useDispatch();
  const LandingData = useSelector(state => state?.home?.homeData);
  const leaderboad = useSelector(state => state?.leaderboard?.leaderboardData);
  const gameEnterStatus = useSelector(
    state => state?.joingameshow?.joingameshowData,
  );
  const livePlans = useSelector(state => state?.livePlans?.livePlanData);
  const Fanjoy_data = useSelector(state => state?.fanjoy?.fanjoyData);
  const wallet = useSelector(state => state?.wallet?.walletData);
  const celebsList = useSelector(state => state?.celebs?.celebsData);
  const cartList = useSelector(state => state?.cart?.cartData);
  const totalLives = useSelector(state => state.user.totalLives);
  ...
  const Livenavigator = async () => {
    if (home2 === null) {
      ...
      await fetch(`${apiBaseUrl}/liveluckydraw`, requestOptions)
  ...
  useEffect(() => {
    ...
    if (isFocused) {
      Livenavigator();
      Homenavigator();
      fanjoyDispatch(fanjoyData());
      walletDispatch(walletData());
      liveplansDispatch(livePlanData());
      celebsListDispatch(celebsData());
      cartDispatch(cartData());
      leaderboardDispatch(leaderboardData());
      homeDispatch(homeData());
      let arr = [];
      LandingData?.data?.host_sliders_data?.map(ele => {
        arr.push(ele.url);
      });
      InAppupdate();
      //for liveluckydraw
  ...
  return (
    <SafeAreaView style={styles.SafeAreaView}>
      ...
      />
      {LandingData?.data && LandingData?.data.updated_version ? (
        <NewVersionmodal
          updatedVersion={LandingData?.data?.updated_version}
          currentV={packageJson.version}
          ModalRef={ModelVersioncheck}
          updatedVersionios={LandingData?.data?.updated_version_ios}
        />
      ) : null}
      {/*<TouchableOpacity
     style={{marginTop: 70}}
        ...
        <View style={styles.mainbody}>
          <LinearGradient
            ...
            <View style={styles.wrap}>
              {LandingData?.loading ? (
                ...
                <Video
                  source={{uri: LandingData?.data?.banners[0]?.url}} // Can be a URL or a local file.
                  resizeMode={'cover'}
                  repeat={true}
                  minLoadRetryCount={2}
                  fullScreen={true}
          ...
          {LandingData?.data?.lowerBanner && (
            <FlatList
              ...
              showsVerticalScrollIndicator={false}
              showsHorizontalScrollIndicator={false}
              data={LandingData?.data?.lowerBanner}
          ...
          {LandingData?.loading ? (
            <ActivityIndicator size="small" color="#000000" />
          ) : LandingData?.data?.home_middle_banners_data ? (
            <HomeCard
              images={LandingData?.data?.home_middle_banners_data}
              time={!LandingData?.loading ? time : 0}
              gameShow={LandingData?.data?.gameShow}
              upcoming_gameshow={LandingData?.data?.upcoming_gameshow}
              countDownFinish={() => {
          ...
          <FlatList
            ...
            showsVerticalScrollIndicator={false}
            showsHorizontalScrollIndicator={false}
            data={LandingData?.data?.products}
            renderItem={({item}) => (
              <ClosingSoonCard
                onPress={() => {
          ...
          <View style={{paddingVertical: 10}}>
            <View style={styles.avatarBannerView}>
              {LandingData?.data?.leaderboard_home_data ? (
                <Image
                  style={[
                  ...
                  source={{
                    uri: LandingData?.data?.leaderboard_home_data[0]?.url,
                  }}
                />
              ) : null}
          ...
          <LinearGradient
            ...
            <FlatList
              data={Fanjoy_data?.data?.data?.allProducts}
              horizontal={true}
              renderItem={({item}) => (
                <Causes
                  onpress={async () => {
                    await dispatch1(causesSlugData(item?.product_slug));
          ...
          {LandingData?.data?.luckydraw_results_data ? (
            <LuckyDrawCard
              image={LandingData?.data?.luckydraw_results_data[0]?.url}
              style={{
                marginTop: 15,
