...
const index = ({props, navigation, route}: any) => {
  ...
  console.log('🚀 ~ index ~ aauser:', user);

  useEffect(() => {
    // if (JSON.stringify(Data) !== user?.data) {
    //   setData(user?.data);
    // }
  ...
  const GetData = async () => {
    ...
    await axios
      .get(`${apiBaseUrl}/livegameshow/user/game/history`, requestOptions)
      .then(response => {
        let res = response;
  ...
  const MyFriends = async () => {
    ...
    // alert(13123);
    await axios
      .get(`${apiBaseUrl}/accepted-connections/list`, requestOptions)
      .then(response => {
