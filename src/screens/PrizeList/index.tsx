...
import { getProducts } from '../../reduxToolkit/actions';
...
const index = ({props, navigation}) => {
  ...
  const productsData = useSelector(state => state?.getproduct?.getProducts);
  ...
  console.log('productsData', productsData?.data?.data);
  ...
  useEffect(() => {
    ...
    getproductDispatch(getProducts('?is_closing_soon=0'))
   // dispatch(getProducts('?is_closing_soon=0'));
    setProductlength(
      productsData?.data?.data?.length ? productsData?.data?.data?.length : '',
    );
    setIsClosing(false);
  }, []);
  const onRefresh = React.useCallback(() => {
    ...
    getproductDispatch(getProducts(isClosing))
    wait(500).then(() => setRefreshing(false));
  }, []);
  const CategoryFunc = (index, id) => {
    let link = `?is_closing_soon=${isClosing ? 1 : 0}&category=${id}`;
    setSelected(index);
    getproductDispatch(getProducts(link))
   // dispatch(getProducts(link));
    setUpdateData(!updateData);
  };
  const NavigateToQuiz = fromSocket => {
    if (
      LandingData?.data?.gameShow?.status === 'on_boarding' ||
      LandingData?.data?.gameShow?.status === 'started' ||
      fromSocket
    ) {
      {
        console.log(
          'LandingData?.data?.gameShow?.status pd',
          LandingData?.data?.gameShow?.status,
        );
      }
      navigation.navigate('GameStack', {
        screen: 'Quiz',
        params: {
          uri: LandingData?.data?.gameShow?.live_stream?.key,
          gameshowStatus: LandingData?.data?.gameShow?.status,
        },
      });
    }
  };
    ...
    <SafeAreaView>
      ...
      <ScrollView
        ...
        <View
          ...
          <LongButton
            onPress={() => {
              getproductDispatch(getProducts('?is_closing_soon=1'))
        ...
        {/* <View>
          <FlatList
            data={productsData?.data?.categories_collection}
            scrollEnabled={true}
