...
import { fanjoyallData } from '../../reduxToolkit/actions/allcreators.actions';
...
const index = ({route, navigation}:any) => {
  ...
  const data = useSelector(state => state.fanjoyall.fanjoyallData);
 // const expData = useSelector(state => state.app.winExperienceProductData);
  //const experienceDetail = useSelector(state => state.fanjoyall.experienceDetail);
  // console.log({winEx: data?.data?.win_experience});
  useEffect(() => {
  ...
  const _Api2 = async id => {
    ...
    await fetch(
      `${apiBaseUrl}/experience/product_list?experience_celebrity_id=${id}`,
      requestOptions,
    )
